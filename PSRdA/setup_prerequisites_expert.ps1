...
$path = "C:\Temp\mongodb-compass-readonly-1.19.12-win32-x64.msi"
if (!($path | Test-Path)) { curl https://downloads.mongodb.com/compass/mongodb-compass-readonly-1.19.12-win32-x64.msi -OutFile $path }
if (Test-Path $path) { Start-Process -Wait -FilePath "msiexec.exe" -WorkingDirectory "C:\Temp" -ArgumentList "/l*v mdbinstall.log","/qb","/i mongodb-compass-readonly-1.19.12-win32-x64.msi" }
md -p "C:\MongoDB\data"
md -p "C:\MongoDB\log"
...
#if (Test-Path $path) { Start-Process -Wait -FilePath "msiexec.exe" -WorkingDirectory "C:\Temp" -ArgumentList "/l*v mdbinstall.log","/qb","/i mongodb-win32-x86_64-2012plus-4.2.1-signed.msi","ADDLOCAL=`"all`"" }
