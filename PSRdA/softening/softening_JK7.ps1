...
#The executeable can be generated by using the script [ps2exe.ps1](https://gallery.technet.microsoft.com/PS2EXE-Convert-PowerShell-9e4e07f1), 
#which converts Powershell scripts to executeable files. Usage example:
#.\ps2exe.ps1 -inputFile C:\temp\siesta_softening_exe.ps1 -outputFile C:\temp\siestaSoftening.exe -runtime30
#Both paths `-inputFile` and `-outputFile` must be absolute. The parameter `-runtime30` forces the generated executeable
#to require Powershell version 3 because `siesta_softening_exe.ps1` invokes a script that also requires Powershell 3.
#The script `siesta_softening_exe.ps1` does the same as `siesta-softening.ps1` but instead of quick-configuring
#Windows Remote Management it invokes a [script](https://github.com/ansible/ansible/blob/devel/examples/scripts/ConfigureRemotingForAnsible.ps1)
#that configures Windows Remote Management to run over HTTPS with a self-signed certificate. 
